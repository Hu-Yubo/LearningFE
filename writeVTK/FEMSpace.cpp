...
////////////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////////////
/// Write VTK data by hyb, lsj and lcy
template<>
void FEMFunction<double, 2>::writeVTKData(const std::string& filename, int flag) const
{
     std::ofstream os(filename.c_str());
	 ...
	 if (flag == 0) {
		 ...
		 int k = mesh.geometry(0, geo.vertex(i)).vertex(0);
		 count[k] += 1;
		 z[k] += value(mesh.point(k), *the_element);
	     }
	     Point<2> bc = (mesh.point(mesh.geometry(0, geo.vertex(0)).vertex(0)) + 
			    mesh.point(mesh.geometry(0, geo.vertex(1)).vertex(0)) +
			    mesh.point(mesh.geometry(0, geo.vertex(2)).vertex(0)));
     ...
     os << "POINTS " << n_node << " double\n";
     for (int i = 0;i < n_node;i ++)
     {
	 os << mesh.point(i) << " " << z[i] << "\n";
     }

     os << "\nCELLS " << n_element << " " << 4 * n_element << "\n";
     the_element = fem_space.beginElement();
     for (int j = 0;the_element != end_element;++ the_element)
     {
	 const GeometryBM& geo = the_element->geometry();
     ...
     os << "\nCELL_TYPES " << n_element << std::endl;
     for (int i = 0; i < n_element; i++)
	 os << 5 << " \n";

     if (flag == 0)
     {
	 os << "\nPOINT_DATA " << n_data << std::endl;
	 os << "SCALARS Point_Value double 1\n";
     }
     else if(flag == 1)
     {
	 os << "\nCELL_DATA " << n_data << std::endl;
	 os << "SCALARS Cell_Value double 1\n";	 
     }
