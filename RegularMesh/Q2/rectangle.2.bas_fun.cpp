...
#define D_ETA_DY ( a[2][1] + a[3][1]*p[0] )
  ...
  void lambda_mm(const double * p, const double ** v, void * value)
  {
    ...
    COMMON_PART;
    val[0] = 1.0 / 4.0 * (1.0 - xi) * xi * (1.0 - eta) * eta;
  }
  ...
  void gradient_lambda_mm(const double * p, const double ** v, void * value)
  {
    double * val = (double *)value;
    COMMON_PART;
    val[0] = 1.0 / 4.0 * ((1.0 - 2.0 * xi) * (1.0 - eta) * eta * D_XI_DX
			  + xi * (1.0 - xi) * (1.0 - 2.0 * eta) * D_ETA_DX);
    val[1] = 1.0 / 4.0 * ((1.0 - 2.0 * xi) * (1.0 - eta) * eta * D_XI_DY
			  + xi * (1.0 - xi) * (1.0 - 2.0 * eta) * D_ETA_DY);
  }
  ...
  void lambda_pm(const double * p, const double ** v, void * value)
  {
    double * val = (double *)value;
    COMMON_PART;
    val[0] = -1.0 / 4.0 * xi * (1.0 + xi) * eta * (1.0 - eta);
  }
  ...
  void gradient_lambda_pm(const double * p, const double ** v, void * value)
  {
    double * val = (double *)value;
    COMMON_PART;
    val[0] = -1.0 / 4.0 * ((2.0 * xi + 1.0) * eta * (1.0 - eta) * D_XI_DX
			   + xi * (xi + 1.0) * (1.0 - 2.0 * eta) * D_ETA_DX);
    val[1] = -1.0 / 4.0 * ((2.0 * xi + 1.0) * eta * (1.0 - eta) * D_XI_DY
			   + xi * (xi + 1.0) * (1.0 - 2.0 * eta) * D_ETA_DY);
  }
  ...
  void gradient_lambda_pp(const double * p, const double ** v, void * value)
  {
    ...
    COMMON_PART;
    val[0] = 1.0 / 4.0 * ((2.0 * xi + 1.0) * eta * (1.0 + eta) * D_XI_DX
			  + xi * (1.0 + xi) * (2.0 * eta + 1.0) * D_ETA_DX);
    val[1] = 1.0 / 4.0 * ((2.0 * xi + 1.0) * eta * (1.0 + eta) * D_XI_DY
			  + xi * (1.0 + xi) * (2.0 * eta + 1.0) * D_ETA_DY);
  }
  ...
  void gradient_lambda_mp(const double * p, const double ** v, void * value)
  {
    double * val = (double *)value;
    COMMON_PART;
    val[0] = -1.0 / 4.0 * ((1.0 - 2.0*xi) * eta * (1.0 + eta) * D_XI_DX
			   + xi * (1.0 - xi) * (2.0 * eta + 1.0) * D_ETA_DX);
    val[1] = -1.0 / 4.0 * ((1.0 - 2.0*xi) * eta * (1.0 + eta) * D_XI_DY
			   + xi * (1.0 - xi) * (2.0 * eta + 1.0) * D_ETA_DY);
  }
  ...
  void lambda_0m(const double * p, const double ** v, void * value)
  {
    double * val = (double *)value;
    COMMON_PART;
    val[0] = 1.0 / 2.0 * (1.0 + xi) * (1.0 - xi) * eta * (eta - 1.0);
  }
  ...
  void gradient_lambda_0m(const double * p, const double ** v, void * value)
  {
    double * val = (double *)value;
    COMMON_PART;
    val[0] = 1.0 / 2.0 * ((-2.0 * xi) * eta * (-1.0 + eta) * D_XI_DX
			  + (1.0 + xi) * (1.0 - xi) * (2*eta - 1.0) * D_ETA_DX);
    val[1] = 1.0 / 2.0 * ((-2.0 * xi) * eta * (-1.0 + eta) * D_XI_DY
			  + (1.0 + xi) * (1.0 - xi) * (2*eta - 1.0) * D_ETA_DY);
  }
  ...
  void lambda_p0(const double * p, const double ** v, void * value)
  {
    double * val = (double *)value;
    COMMON_PART;
    val[0] = 1.0 / 2.0 * xi * (1.0 + xi) * (1.0 + eta) * (1.0 - eta);
  }
  ...
  void gradient_lambda_p0(const double * p, const double ** v, void * value)
  {
    double * val = (double *)value;
    COMMON_PART;
    val[0] = 1.0 / 2.0 * ((2.0 * xi + 1.0) * (1.0 - eta * eta) * D_XI_DX
			  + xi * (1.0 + xi) * (-2.0 * eta) * D_ETA_DX);
    val[1] = 1.0 / 2.0 * ((2.0 * xi + 1.0) * (1.0 - eta * eta) * D_XI_DY
			  + xi * (1.0 + xi) * (-2.0 * eta) * D_ETA_DY);
  }
  ...
  void lambda_0p(const double * p, const double ** v, void * value)
  {
    double * val = (double *)value;
    COMMON_PART;
    val[0] = 1.0 / 2.0 * (1.0 + eta) * eta * (1.0-xi*xi);
  }
  ...
  void gradient_lambda_0p(const double * p, const double ** v, void * value)
  {
    double * val = (double *)value;
    COMMON_PART;
    val[0] = 1.0 / 2.0 * ((-2.0 * xi) * (eta + eta * eta) * D_XI_DX
			  + (1.0 - xi * xi) * (1.0 + 2.0 * eta) * D_ETA_DX);
    val[1] = 1.0 / 2.0 * ((-2.0 * xi) * (eta + eta * eta) * D_XI_DY
			  + (1.0 - xi * xi) * (1.0 + 2.0 * eta) * D_ETA_DY);
  }
  ...
  void lambda_m0(const double * p, const double ** v, void * value)
  {
    double * val = (double *)value;
    COMMON_PART;
    val[0] = 1.0 / 2.0 * (xi - 1.0) * (xi) * (1.0 - eta * eta);
  }
  ...
  void gradient_lambda_m0(const double * p, const double ** v, void * value)
  {
    ...
    COMMON_PART;
    val[0] = 1.0 / 2.0 * ((2.0 * xi - 1.0) * (1.0 - eta * eta) * D_XI_DX
			  + (-1.0 + xi) * xi * (-2.0 * eta) * D_ETA_DX);
    val[1] = 1.0 / 2.0 * ((2.0 * xi - 1.0) * (1.0 - eta * eta) * D_XI_DY
			  + (-1.0 + xi) * xi * (-2.0 * eta) * D_ETA_DY);
  }
